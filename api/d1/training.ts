/**
 * D1 Training Examples API (Fine-tuning)
 */
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const result = await d1Query(
        `INSERT INTO training_examples (input_text, output_text, category, quality_score) VALUES (?, ?, ?, ?)`,
        [body.input_text, body.output_text, body.category || "general", body.quality_score || 5]
      );
    ...
    if (req.method === 'DELETE') {
      ...
      await d1Query("DELETE FROM training_examples WHERE id = ?", [parseInt(id)]);
      return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
    }

    return new Response(JSON.stringify({ error: 'Method not allowed' }), { status: 405, headers: corsHeaders });
  } catch (error: any) {
    console.error('[D1 Training] Error:', error);
