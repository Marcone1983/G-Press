/**
 * D1 Documents API - Knowledge Base
 */
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const result = await d1Query(
        ...
        [body.title, body.content, body.type || "document", body.category || "general", JSON.stringify(body.tags || [])]
      );
    ...
    if (req.method === 'DELETE') {
      ...
      await d1Query("DELETE FROM knowledge_documents WHERE id = ?", [parseInt(id)]);
      return new Response(JSON.stringify({ success: true }), { headers: corsHeaders });
    }

    return new Response(JSON.stringify({ error: 'Method not allowed' }), { status: 405, headers: corsHeaders });
  } catch (error: any) {
    console.error('[D1 Documents] Error:', error);
